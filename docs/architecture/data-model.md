...
├── pagination.json           ← page-size/count metadata for index.json's arrays
...
Metadata for paging through `index.json`'s `daily`/`weekly`/`monthly` arrays. There is no server, so this doesn't serve page N directly — it just publishes the page size and counts so a consumer can slice the arrays itself: `daily.slice(page * page_size, (page + 1) * page_size)`. Regenerated by the export step and by the weekly/monthly recap step every time `index.json` changes.

```json
{
  "updated_at": "2026-07-08T02:06:03Z",
  "page_size": 30,
  "sections": {
    "daily": { "total_items": 1, "total_pages": 1 },
    "weekly": { "total_items": 0, "total_pages": 0 },
    "monthly": { "total_items": 0, "total_pages": 0 }
  }
}
```
...
| Field | Notes |
|-------|-------|
| `page_size` | Number of index entries per page (fixed at 30) |
| `sections.<key>.total_items` | Length of `index.json`'s `<key>` array |
| `sections.<key>.total_pages` | `ceil(total_items / page_size)`; `0` when the section is empty |
