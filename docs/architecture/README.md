...
graph TB
    ...
    consumer["Third-party frontend / consumer"]
    ...
    json -->|"fetch raw JSON\n(raw.githubusercontent.com / jsDelivr)"| consumer
```
...
### Stage 4 — Publish
There is no serving step of our own. Any consumer — a third-party frontend, script, or service — fetches `data/latest.json` or any historical daily/weekly/monthly file directly via `raw.githubusercontent.com` (or jsDelivr for CDN caching). No API server, no deploy step, no our-side hosting. See [Consuming the API](consuming-the-api.md).
...
| **Data-only** | Veen ships no UI; consumers bring their own frontend against the published JSON |
| **Zero infra ops** | No K8s, no VMs, no databases, no hosting to maintain or pay for |
